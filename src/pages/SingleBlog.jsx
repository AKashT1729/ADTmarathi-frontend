...
const SingleBlog = () => {
  ...
  if (!blog) return <div className="text-center mt-20 text-lg text-gray-500">No blog data found.</div>;
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center px-2 py-6">
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-lg p-6 mb-8">
        ...
        <p className="text-gray-400 text-xs mb-4">
          ADTMarathi &middot; {blog.createdAt ? new Date(blog.createdAt).toLocaleDateString() : ''} &middot; <span className="italic">Blog</span>
        </p>
        ...
        <div className="text-gray-700 text-base mb-6 leading-7">
          {paragraphs.length > 0 ? (
            paragraphs.map((para, idx) => (
              <p key={idx} className={idx !== paragraphs.length - 1 ? "mb-4" : ""}>{para}</p>
            ))
          ) : (
